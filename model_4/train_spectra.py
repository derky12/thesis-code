    ...
    with torch.no_grad():
        if config.in_training:
            ...
            for (
                ...
                trg_input,
                trg_output,
            ) in sampled_datapoints:
                integer_parts = integer_parts.unsqueeze(0).to(device)
                fractional_parts = fractional_parts.unsqueeze(0).to(device)
                intensity_parts = intensity_parts.unsqueeze(0).to(device)
                ...
                pred_encoder_output, _ = spectra_model(
                    (integer_parts),
                    (fractional_parts),
                    (intensity_parts),
                    precursor_mass,
                ...
                if pred_smi != "invalid":
                    if decod_truth == pred_smi:
                        ...
                        tanimoto_ = FpSimilarity(
                            decod_truth, pred_smi, radius=2, nBits=2048
                        )
            ...
            df.to_csv(f"./pred_smiles_{config.training_id}.csv")
            spectra_model.train()
...
class SpectraDataset(Dataset):
    ...
    def __getitem__(self, idx):
        spec, smi, precursor_mass, ais_tokens = self.dataset[idx]
