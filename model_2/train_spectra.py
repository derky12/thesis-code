...
def validation_loop(model, dataloader, config):
    errors = []
    model.eval()
    ...
    with torch.no_grad():
        for step, batch in enumerate(dataloader):
            ...
            integer_parts = (integer_parts.to(device),)
            fractional_parts = (fractional_parts.to(device),)
            intensity_parts = (intensity_parts.to(device),)
            encoder_output = h.to(device).squeeze(1)  # (batch_size, d_model)

            preds, _ = model(
                integer_parts, fractional_parts, intensity_parts, precursor_mass
            )  # (batch_size, d_model)
            error = torch.mean((preds - encoder_output) ** 2)
...
class SpectraDataset(Dataset):
    def __init__(self, csv_path, trans_model, config):
        self.trans_model = trans_model
        self.config = config
        df = pd.read_csv(csv_path, low_memory=False)

        spec_data = df["spectrum"]
        smi_data = df["cano_smi"]
        precursor_mass_data = df["precursor_mz"]
        self.dataset = []
        for i in range(len(smi_data)):
            item = [spec_data[i], smi_data[i], precursor_mass_data[i]]
    ...
    def tokenize(self, fragment_mz, intensities):
        # Sort m/z values by intensity
        sorted_indices = torch.argsort(intensities, descending=True)
        sorted_fragment_mz = torch.gather(fragment_mz, 0, sorted_indices)
        sorted_intensities = torch.gather(intensities, 0, sorted_indices)

        integer_parts = (sorted_fragment_mz // 1).long()
        fractional_parts = ((sorted_fragment_mz % 1) * 1000).long()

        integer_parts = F.pad(
            integer_parts, (0, self.config.spectra_vocab_size - len(integer_parts))
        )
        fractional_parts = F.pad(
            fractional_parts,
            (0, self.config.spectra_vocab_size - len(fractional_parts)),
        )
        intensity_parts = F.pad(
            sorted_intensities,
            (0, self.config.spectra_vocab_size - len(sorted_intensities)),
        )

        return integer_parts, fractional_parts, intensity_parts

    def __getitem__(self, idx):
        spec, smi, precursor_mass = self.dataset[idx]
        ...
        for s in spec_l:
            l = s.split(":")
            mz_l.append(round(float(l[0]), 4))
            int_l.append(round(float(l[1]), 4))
        mz_l = torch.tensor(mz_l)
        int_l = torch.tensor(int_l)
        integer_parts, fractional_parts, intensity_parts = self.tokenize(mz_l, int_l)
        smiles_tensor_list = [
            self.trans_model.string2tensor(smiles) for smiles in [smi]
        ]
        h = self.trans_model.forward_encoder(smiles_tensor_list)

        return (
            integer_parts.float(),
            fractional_parts.float(),
            intensity_parts.float(),
            smi,
            precursor_mass,
            h,
        )
...
def train(config):
    ...
    train_set = SpectraDataset(config.spectra_train_path, trans_model, config)
    ...
    val_set = SpectraDataset(config.spectra_val_path, trans_model, config)
    ...
    print("Loading model...")
    model = SpectraEncoder(precursor_mass_mask_ratio=config.precursor_mass_mask_ratio)
    ...
    for epoch in range(start_epoch, n_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                # print(
                #     f"{integer_parts.shape=}, {fractional_parts.shape=}, {intensity_parts.shape=},  {precursor_mass.shape=}"
                # )
                integer_parts = (integer_parts.to(device),)
                fractional_parts = (fractional_parts.to(device),)
                intensity_parts = (intensity_parts.to(device),)
                encoder_output = h.to(device).squeeze(1)
                # print(f"{encoder_output.shape=}")

                precursor_embedded = (
                    precursor_mass.clone().detach().unsqueeze(1).to(device)
                )

                precursor_embedded = F.pad(precursor_embedded, (0, config.d_model - 1))
                # print(f"{precursor_embedded.shape=},{encoder_output.shape=}")
                smi_encoder_output = torch.stack(
                    (precursor_embedded, encoder_output), dim=1
                )
                # print(f"{smi_encoder_output.shape=}")

                _, preds = model(
                    integer_parts, fractional_parts, intensity_parts, precursor_mass
                )
                # print(f"{preds.shape=},{preds.dtype=}")
                loss = criterion(preds, smi_encoder_output)
                optimizer.zero_grad()
        ...
        val_error = validation_loop(model, val_dataloader, config)
