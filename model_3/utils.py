...
def build_model(model_type):
    ...
    return Transformer(src_vocab_size=len(src_i2w), trg_vocab_size=len(trg_i2w)).to(
        device
    )
...
def get_data_loader(model_type, file_name, batch_size=100, shuffle=True):
    ...
    with open(
        f"{DATA_DIR}/{SRC_DIR}/{model_type}_{file_name}", "r", encoding="utf-8"
    ) as f:
        src_text_list = f.readlines()

    with open(
        f"{DATA_DIR}/{TRG_DIR}/{model_type}_{file_name}", "r", encoding="utf-8"
    ) as f:
...
class CustomDataset(Dataset):
    def __init__(self, src_list, input_trg_list, output_trg_list):
        ...
        assert np.shape(src_list) == np.shape(
            input_trg_list
        ), "The shape of src_list and input_trg_list are different."
        assert np.shape(input_trg_list) == np.shape(
            output_trg_list
        ), "The shape of input_trg_list and output_trg_list are different."
...
def CalcFpTc(truth_smi, pred_smi, metric=Chem.DataStructs.TanimotoSimilarity):
    ...
    return DataStructs.TanimotoSimilarity(
        AllChem.GetMorganFingerprintAsBitVect(truth_mol, 2, nBits=2048),
...
def AesEncoder(smiles, radius):
    ...
    for atom in mol.GetAtoms():
        atomID_sma[atom.GetIdx()] = getSubstructSmi(mol, atom.GetIdx(), radius)[
            1
        ]  # by [1] selecting SMARTS
...
def AisEncoder(smiles, radius=0):
    ...
    for smiles in smiles_list:
        ...
        for atom in mol.GetAtoms():
            atomID_sma[atom.GetIdx()] = getAis(
                mol, atom.GetIdx(), tmp_radius
            )  # [1] # by [1] selecting SMARTS
        ...
        regex = re.compile(pattern)
        tokens_list = [token for token in regex.findall(Chem.MolToSmiles(mol))]
        assert Chem.MolToSmiles(mol) == "".join(
            tokens_list
...
def getAtomEnvs(smiles, radii=[0, 1], radius=1, nbits=1024):
    ...
    assert (
        max(radii) <= radius
    ), f"the maximum of radii should be equal or lower than radius, but got {max(radius)}"
    ...
    fp = AllChem.GetMorganFingerprintAsBitVect(
        molP, radius=radius, nBits=nbits, bitInfo=info
    ...
    for k, j in enumerate(updateInfoTemp):
        tokens_str += (
            str(updateInfoTemp[k][2]) + " "
        )  # [2]-> selecting SMARTS description
...
def tanimoto(truth, prediction, i, j):
    return len(set(truth[i]) & set(prediction[j])) / float(
        len(set(truth[i]) | set(prediction[j]))
    )
...
def timing(f):
    ...
    def wrap(*args, **kwargs):
        ...
        print(
            f"{f.__name__} -> Elapsed time: {hours}hrs {minutes}mins {seconds:.3}secs"
        )
...
def mp_dbSearch(results_dict, dbdir, topk=5):
    ...
    # return results
    return pd.DataFrame(
        results, columns=["Tree", "Model_Prediction", "DB_AEs", "DB_SMILES", "DB_Tc"]
    )
