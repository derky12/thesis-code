...
def custom_validation_fn(
    ...
    total, exact, _100, _90, _85, _80, _70, _60, _50, _40, _30, _20, _00, _invalid = (
        0,
        0,
        0,
        0,
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            for j in range(len(src_input)):
                ...
                encoder_mask = (
                    (src_j != pad_id).unsqueeze(1).to(device)
                )  # (1, L) => (1, 1, L)
                # encoding src input
    ...
    logger.info(
        f"{total, exact/total, _100/total, exact, _100, _90, _85, _80, _70, _60, _50, _40, _30, _20, _00, _invalid} \t{elapsed_time=}"
    )
    ...
    for pos in range(seq_len):
        ...
        for k in range(beam_size):
            ...
            if node.is_finished:
                ...
                d_mask = (
                    (trg_input.unsqueeze(0) != pad_id).unsqueeze(1).to(device)
                )  # (1, 1, L)
                nopeak_mask = torch.ones([1, seq_len, seq_len], dtype=torch.bool).to(
                    device
                )
...
def main(args):
    if args.test_mode:
        test_loader = get_data_loader(
            args.model_type, TEST_NAME, args.batch_size, shuffle=False
        )

        if args.checkpoint_name:
            model = setup(build_model(model_type=args.model_type), args.checkpoint_name)
            _step = re.search(
                f"{args.model_type}_checkpoint_epoch_(.*).pth", args.checkpoint_name
            ).group(1)
            ...
            custom_validation_fn(
                model, test_loader, args.model_type, method=args.decode
            )
            ...
            for ckpt in Path(ckpt_dir).iterdir():
                if ckpt.name.startswith(f"{args.model_type}_checkpoint_epoch_"):
                    print(ckpt)
                    _step = re.search(
                        f"{args.model_type}_checkpoint_epoch_(.*).pth", ckpt.name
                    ).group(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        type=str,
        required="--test_mode" not in sys.argv,
        help="An input sequence",
    )
    ...
    parser.add_argument(
        "--checkpoint_name", type=str, default=None, help="checkpoint file name"
    )
    ...
    logger.info(args)
    assert (
        args.decode == "greedy" or args.decode == "beam"
